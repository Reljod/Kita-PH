...
async def run_org_scaffolding(org_id: str):
    ...
    agent_service = AgentService(
        llm_service=llm_service, collection=agent_coll, tools_collection=tools_coll
    )

    web_search_service = SerperSearchService()
    tool_service = ToolService(
        web_search_service=web_search_service, collection=tools_coll
    )
    ...
    try:
        ...
        logger.info(
            f"Successfully completed organization scaffolding for org_id: {org_id}"
        )
    except Exception as e:
        logger.error(
            f"Error running organization scaffolding for {org_id}: {e}", exc_info=True
        )
...
async def create_organization(
    ...
    # OrgCreate has org_name/org_code, not `name` — reading `.name` raised an
    # AttributeError on the very first line, so every attempt to create an
    # organization answered 500 before any work was done.
    logger.info(
        f"Attempting to create organization: {org_in.org_name} by user: {current_user.id}"
    )
...
async def get_org_creation_status(
    ...
    return {"org_id": org.id, "status": org.status or "completed"}
...
async def add_or_update_member(
    ...
    if id != authorized_org_id:
        ...
        if not org or org.id != authorized_org_id:
            logger.warning(
                f"Access denied adding member to org {id} by user {current_user.id}"
            )
            raise HTTPException(
                status_code=403, detail="Access denied to this organization"
            )
    ...
    logger.info(
        f"Adding/updating member {member_in.user_id} with role {member_in.role} in org {authorized_org_id}"
    )
    ...
    if not org:
        ...
        raise HTTPException(status_code=404, detail="Organization not found")
    logger.info(
        f"Successfully added/updated member {member_in.user_id} in org {authorized_org_id}"
    )
    return org
...
async def revoke_member(
    ...
    if id != authorized_org_id:
        ...
        if not org or org.id != authorized_org_id:
            logger.warning(
                f"Access denied revoking member from org {id} by user {current_user.id}"
            )
            raise HTTPException(
                status_code=403, detail="Access denied to this organization"
            )
