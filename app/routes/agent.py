...
async def create_agent(
    req: AgentCreateRequest, service: IAgentService = Depends(get_agent_service)
):
    try:
...
def get_all_agents(
    last_chat: bool = False, service: IAgentService = Depends(get_agent_service)
):
...
async def create_agent_chat(
    ...
    async def event_generator():
        try:
            async for event in chat_service.create_chat_stream(
                req, agent_id=agent_id, status_key=x_status_key
            ):
...
async def continue_agent_chat(
    ...
    async def event_generator():
        try:
            async for event in chat_service.continue_chat_stream(
                chat_id, req, agent_id=agent_id, status_key=x_status_key
            ):
                yield f"data: {json.dumps(event)}\n\n"
        except Exception as e:
            err_data = {"type": "error", "message": str(e)}
...
async def get_agent_chat(
    agent_id: str, chat_id: str, chat_service: IChatService = Depends(get_chat_service)
):
    try:
...
async def create_agent_rag(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error creating agent memory: {str(e)}"
        )


@router.get("/{agent_id}/memory", response_model=List[RagResponse])
async def get_all_agent_rags(
    agent_id: str, rag_service: IRagService = Depends(get_agent_rag_service)
):
...
async def search_agent_memory(
    ...
    limit: int = Query(
        5, ge=1, le=50, description="The maximum number of results to return."
    ),
    rag_service: IRagService = Depends(get_agent_rag_service),
):
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error searching agent memory: {str(e)}"
        )


@router.get("/{agent_id}/memory/{rag_id}", response_model=RagResponse)
async def get_agent_rag(
    agent_id: str,
    rag_id: str,
    rag_service: IRagService = Depends(get_agent_rag_service),
):
...
async def update_agent_rag(
    ...
    # intended status through untouched.
        ...
        raise HTTPException(
            status_code=500, detail=f"Error updating agent memory: {str(e)}"
        )
...
async def delete_agent_rag(
    ...
    # intended status through untouched.
        ...
        raise HTTPException(
            status_code=500, detail=f"Error deleting agent memory: {str(e)}"
        )
