...
class ServiceRegistry:
    def __init__(self, org_id: str):
        ...
        self.llm_service = LlmService(
            TenantCollection(db.get_llms_collection(), org_id)
        )
        ...
        self.graph_rag_service = Neo4JGraphRagService(
            neo4j_uri, neo4j_user, neo4j_password, org_id
        )
        ...
        self.mongodb_vector_search_rag_service = MongoDBVectorSearchRagService(
            collection=TenantCollection(
                db.get_file_parsed_flattened_collection(), org_id
            )
        )
...
def get_graph_rag_service(
    ...
    if not all([neo4j_uri, neo4j_user, neo4j_password]):
        raise HTTPException(
            status_code=500,
            detail="Graph RAG environment variables are not properly configured.",
        )
...
def get_agent_rag_service(
    agent_id: str, org_id: str = Depends(get_current_org_id)
) -> IRagService:
    return get_services(org_id).rag_service
...
def get_ingest_service(org_id: str = Depends(get_current_org_id)) -> IIngestService:
...
def get_adaptive_rag_service(org_id: str = Depends(get_current_org_id)) -> Any:
    return get_services(org_id).adaptive_rag_service


def get_agent_status_service(org_id: str = Depends(get_current_org_id)) -> Any:
