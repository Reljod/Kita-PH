...
from app.models.graph_rag import (  # noqa: E402  (kept below the logger for import-order reasons)
    GraphNode,
    GraphRelationship,
    GraphDocument,
    GraphChunk,
...
class GraphRagService(Protocol):
    async def ingest_document(
        self, doc: GraphDocument, chunks: List[GraphChunk]
    ) -> bool: ...
    ...
    async def query(
        self, query_text: str, limit: int = 5, filters: Optional[Dict[str, Any]] = None
...
class Neo4JGraphRagService:
    ...
    def _safe_identifier(value: str, kind: str) -> str:
        """Validate a label or relationship type before it enters Cypher.

        Cypher cannot parameterize labels or relationship types, so they are
        interpolated into the query string. Anything that is not a plain
        identifier is therefore executable, which makes an unvalidated caller
        an injection vector rather than a bad argument.
        """
    ...
    def _prepare_props(self, props: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for k, v in props.items():
            # Neo4j properties are primitives or flat arrays of primitives; a
            # map or a nested array is rejected by the driver. The previous
            # condition iterated `[]` for a dict, so `all(...)` was vacuously
            # true and no mapping was ever serialised.
            ...
            for chunk in chunks:
                ...
                if not chunk.embedding:
                    ...
                    emb = await loop.run_in_executor(
                        None, self.model.encode, chunk.content
                    )
    ...
    async def query(
        ...
        truncated_query = (
            query_text[:150] + "..." if len(query_text) > 150 else query_text
        )

        # Calculate embedding
        loop = asyncio.get_event_loop()
        query_embedding = await loop.run_in_executor(
            None, self.model.encode, query_text
        )
                    ...
                    if value is not None:
                        ...
                        if key == "agent_id":
                            filter_clauses.append(
                                f"(chunk.{key} = ${param_name} OR chunk.{key} IS NULL)"
                            )
                        else:
                ...
                for n in record["entities"] + record["neighbors"]:
                    if n.get("id"):
                        ...
                        for pk, pv in raw_props.items():
                            ...
                            elif isinstance(pv, str) and (
                                pv.startswith("{") or pv.startswith("[")
                            ):
                                try:
                        ...
                        nodes.append(
                            GraphNode(id=n["id"], label=n["label"], properties=props)
                        )
    ...
    async def upsert_node(self, label: str, properties: Dict[str, Any]) -> str:
        ...
        label = self._safe_identifier(label, "node label")

        async with self.driver.session() as session:
            # Labels cannot be parameterized in Cypher, so this is interpolated
            # -- validated above rather than trusted.
            query = f"MERGE (n:{label} {{id: $id, org_id: $org_id}}) SET n += $props RETURN n.id"
            res = await session.run(
                query,
                id=properties["id"],
                org_id=self.org_id,
                props=self._prepare_props(properties),
            )
    ...
    async def upsert_relationship(
        ...
        rel_type = self._safe_identifier(rel_type, "relationship type")
