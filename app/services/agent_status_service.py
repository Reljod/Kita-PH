...
MESSAGES_MAP = {
    ...
    "draft_response": ["drafting a response", "formulating the response draft"],
    "finalize_response": ["finalizing the answer", "writing the final reply"],
    "delegated_task": [
...
class AgentStatusService:
    def __init__(
        self, org_id: str, redis_client: aioredis.Redis, agent_service: Any = None
    ):
        self.org_id = org_id
    ...
    # The status key arrives verbatim from the client's `x-status-key` header,
    # so it is neither unguessable nor unique across tenants. Without the
    # org_id in the key, one organization reading `GET /chat/status/<key>` --
    # or opening the status WebSocket -- gets another organization's run: its
    # agent id, its chat id, and the step messages naming the agent. Every
    # Mongo read goes through TenantCollection for exactly this reason; the
    # Redis-backed status store has to carry the same boundary itself.
    def _get_redis_key(self, status_key: str) -> str:
        return f"agent_status:{self.org_id}:{status_key}"

    def _get_channel_name(self, status_key: str) -> str:
        return f"agent_status:channel:{self.org_id}:{status_key}"
    ...
    async def start_session(
        self, status_key: str, agent_id: str, chat_id: Optional[str] = None
    ) -> AgentStatus:
    ...
    async def update_step(
        self, status_key: str, step: str, agent_id: Optional[str] = None
        ...
        new_step = AgentStepStatus(
            step=step, message=message, agent_id=active_agent, started_at=now_str
        )
        ...
        await self._save_and_publish(status, ttl=3600)
        logfire.info(
            "Updated status step: key={key}, step={step}", key=status_key, step=step
        )
        return status

    async def finish_session(
        self, status_key: str, success: bool = True, chat_id: Optional[str] = None
    ) -> Optional[AgentStatus]:
        status_dict = await self.get_status(status_key)
        if not status_dict:
            return None
        ...
        logfire.info(
            "Finished status session: key={key}, success={success}",
            key=status_key,
            success=success,
        )
        return status
