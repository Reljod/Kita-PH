...
def format_chat_response(
    doc: Dict[str, Any], preview_only: bool = False
) -> ChatResponse:
    ...
    if messages:
        ...
        if isinstance(first_msg, dict):
            if "parts" in first_msg:
                # Only some part kinds carry `content` — a tool-call part
                # holds `args` instead — so index defensively. A KeyError here
                # would take down the whole chat list, not just one preview.
                content = "\n".join(
                    [
                        p.get("content", "")
                        for p in first_msg["parts"]
                        if isinstance(p, dict) and p.get("part_kind") != "thinking"
                    ]
                )
            else:
...
class ChatService(IChatService):
    ...
    async def create_chat(
        ...
        new_chat = ChatDocument(messages=messages_dump, agent_id=base_agent_id)
    ...
    async def continue_chat(
        ...
        self.collection.update_one({"_id": obj_id}, {"$set": update_fields})
    ...
    async def create_chat_stream(
        ...
        if final_result is None:
            raise AgentRunStreamFailedError(
                agent_id or "unknown", "No run result returned from agent run stream"
            )
    ...
    async def continue_chat_stream(
        ...
        if final_result is None:
            raise AgentRunStreamFailedError(
                agent_to_run_with or "unknown",
                "No run result returned from agent run stream",
            )
