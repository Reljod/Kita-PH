...
async def create_agent(
    ...
    llm_id: Annotated[
        Optional[str],
        Field(
            description="The ID of the LLM model to use for this agent. If not provided, it will be automatically selected based on environment defaults."
        ),
    ] = None,
) -> str:
    """
    ...
    if not llm_id or llm_id == "null":
        ...
        match = next((m for m in available_llms if m.model == env_model), None)
        if not match:
            # Try to match with grok-4.3 fallback
            match = next(
                (m for m in available_llms if m.model == "x-ai/grok-4.3"), None
            )

        if not match:
            # AgentCreateRequest requires a non-empty llm_id, so falling
            # through with "" raised a pydantic ValidationError -- an opaque
            # 500-shaped failure for what is really "this organization has no
            # models registered". Tools answer the model in prose, so say it.
            available = ", ".join(sorted(llm.model for llm in available_llms))
            return (
                "Could not create the agent: no LLM is registered for this "
                "organization to use as a default. Ask the user to pick one "
                f"and pass its llm_id. Available models: {available or 'none'}."
            )
        llm_id = match.id
...
async def get_agent(
    ...
    agent_id: Annotated[
        str,
        Field(
            description="The ID of the agent to get (e.g., '67bc...-v1' or '67bc...')"
        ),
    ],
) -> str:
    """
    ...
    try:
        ...
        # get_llm raises on anything that is not an ObjectId, and an agent can
        # carry a stale or hand-written llm_id. Describing the agent is the
        # point of this tool; an unreadable model id should not stop it.
        llm = None

    llm_info = (
        f"{llm.name} ({llm.model} via {llm.provider})"
        if llm
        else f"Unknown LLM (ID: {agent.llm_id})"
    )
...
@creator_toolset.tool
@log_tool_call
async def list_agents(ctx: RunContext[dict]) -> str:
    """
...
async def update_agent(
    ...
    name: Annotated[
        Optional[str], Field(description="The new name of the agent.")
    ] = None,
    role: Annotated[
        Optional[str], Field(description="The new role of the agent.")
    ] = None,
    goal: Annotated[
        Optional[str], Field(description="The new goal of the agent.")
    ] = None,
    backstory: Annotated[
        Optional[str], Field(description="The new backstory of the agent.")
    ] = None,
    personalities: Annotated[
        Optional[List[str]],
    ...
    new_version: Annotated[
        bool,
        Field(
            description="Whether to create a new version (True) or update the current version in place (False). Set to False for iterating/tweaking."
        ),
    ] = True,
) -> str:
    """
    ...
    updated_agent = await service.update_agent(
        agent_id, update_req, new_version=new_version
    )
