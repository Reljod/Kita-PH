...
class AuthService:
    ...
    def verify_token(self, token: str) -> Optional[TokenData]:
        try:
            ...
            if user_id is None:
                logger.warning(
                    "Token verification failed: sub (user_id) missing in payload"
                )
                return None
            ...
            if not token_doc:
                logger.warning(
                    f"Token verification failed: token is revoked or not found in db (user_id: {user_id})"
                )
                return None
    ...
    def generate_tokens(self, user_id: str, org_id: Optional[str] = None) -> Token:
        ...
        db.get_tokens_collection().insert_one(token_doc.model_dump())
    ...
    def revoke_user_tokens(self, user_id: str):
        logger.info(
            f"Revoking all tokens for user_id={user_id}", extra={"user_id": user_id}
        )
        db.get_tokens_collection().update_many(
