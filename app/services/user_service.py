...
def _to_object_id(user_id: str) -> Optional[ObjectId]:
    """Parse a user id, or None when it isn't a valid ObjectId.

    The id reaching these methods comes from a JWT `sub` claim or a path
    parameter, so it is attacker-influenced. Letting bson.InvalidId escape
    turns a bad identifier into a 500; callers treat None as "no such user"
    and answer 401/404 instead.
    """
    try:
...
class UserService:
    ...
    def create_user(self, user_in: UserCreate, prehashed: bool = False) -> UserResponse:
        ...
        hashed_password = (
            user_in.password if prehashed else self.hash_password(user_in.password)
        )
    ...
    def update_password(self, user_id: str, password_in: PasswordUpdate) -> bool:
        ...
        if not user or not self.verify_password(
            password_in.old_password, user["password"]
        ):
            return False
